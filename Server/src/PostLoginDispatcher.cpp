...
PostLoginDispatcher::PostLoginDispatcher() {
	currSock = NULL;
	isAlive = true;
	start();
}

void PostLoginDispatcher::updateListener() {
...
int PostLoginDispatcher::sendCommand(TCPSocket* socket, const char* content,
		int command) {
...
void PostLoginDispatcher::addPeer(char* user, TCPSocket* peer) {
	Guard guard(&mutex);
	cout << "New peer added to the ClosedDispatcher: " << peer->toString()
			<< endl;
	int cnt = 0;
	char* temp = user;
	while (*user != ':') {
...
void PostLoginDispatcher::pushAllScores(map<string, int> namesAndScores) {
	Guard guard(&mutex2);
	ofstream ofs;
...
map<string, int> PostLoginDispatcher::getAllScores() {
	Guard guard(&mutex2);
...
void PostLoginDispatcher::run() {
	...
	while (isAlive) {
		...
		case SEND_REQUEST: {
			...
			if (name[0] == 0) {
				cout << "REjected" << endl;
				sendCommand(currSock, NULL, REJECT_REQUEST);
			} else {
				name[size] = '\0';
				for (map<string, TCPSocket*>::iterator it = onlinePeers.begin();
						it != onlinePeers.end(); ++it) {
					if ((it->second) == currSock) {
						sendCommand(onlinePeers[name], (it->first).c_str(),SEND_REQUEST);
						flag = true;
						break;
					}
				}
		...
		case ACCEPT_REQUEST: {
			...
			for (map<string, TCPSocket*>::iterator it = onlinePeers.begin();
				...
				if ((it->second) == currSock) {
					char port1[5];
					char port2[5];
					sprintf(port1, "%d", currSock->getPort()); //getLocalPort
					sprintf(port2, "%d", onlinePeers[name]->getPort());//getLocalPort
		...
		case REJECT_REQUEST: {
			...
			for (map<string, TCPSocket*>::iterator it = onlinePeers.begin();
					it != onlinePeers.end(); ++it) {
				if ((it->second) == currSock) {
					sendCommand(onlinePeers[name], (it->first).c_str(),REJECT_REQUEST);
					flag = true;
					break;
				}
			}
		...
		case REQUEST_SCOREBOARD: {
			...
			for (unsigned int i = 0; i < newSocres.size(); i++) {
