...
void ClientCLI::runCLI() {
	...
	while (!loggedIn) {
		...
		case 'r': {
			...
			client.registerUser((char*) userName.c_str(), 555); // UNSIGNED INT PROB
			sleep(1);
			if (lastCmd == REGISTER_SUCCESSFUL) {
				cout << "Registration succeed, logging in...." << endl;
				client.LogInUser((char*) userName.c_str(), 555) /** instead of hashed password*/;
				sleep(1);
		...
		case 'l': {
			...
			do {
				...
				cout << "Enter password: " << endl;
				cin >> password;
				client.LogInUser((char*) userName.c_str(), 555);// UNSIGNED INT PROB
				sleep(1);
				if (lastCmd == LOGIN_SUCCESSFUL) {
					cout << "You have entered the game lobby!" << endl;
					loggedIn = true;
