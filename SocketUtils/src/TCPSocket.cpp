...
string TCPSocket::toString()
{
	...
	sprintf(addr, "%d.%d.%d.%d:%d",
			int(this->peerAddr.sin_addr.s_addr & 0xFF),
			int((this->peerAddr.sin_addr.s_addr & 0xFF00) >> 8),
			int((this->peerAddr.sin_addr.s_addr & 0xFF0000) >> 16),
			int((this->peerAddr.sin_addr.s_addr & 0xFF000000) >> 24),
			ntohs(this->peerAddr.sin_port));
	return addr;
}
